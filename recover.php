		...
		if(!empty($email)){
			if(!Validate::filterEmail($email)){
				...
				if($user = $userObj->emailExist($email)){
					...
					$message  = "{$user->firstName}, Someone requested for new password, if you didn't requested new password then leave this email, however if you did then here's your reset link <a href='http://localhost:10080/mastersProject/cte-ems/account/recover/{$link}'>Reset Password</a>";
